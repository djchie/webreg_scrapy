    ...
    def parse(self, response):
        ...
        for courseXML in response.xpath('//tr[@bgcolor="#fff0ff"]'):
            ...
            for sessionXML in courseXML.xpath('following-sibling::tr[@valign="top" and count(preceding-sibling::tr[@class="blue-bar"])=' + str(blueBarCount) + ']'):
                ...
                # This column was removed recently
                # session['reservedForNewStudentsCount'] = sessionXML.xpath('td[12]/text()').extract()[0].replace(u"\u00A0", " ")
                session['enrollmentRestrictions'] = sessionXML.xpath('td[12]/text()').extract()[0].replace(u"\u00A0", " ")

                if len(sessionXML.xpath('td[13]/a')) == 1:
                    session['textbookLink'] = sessionXML.xpath('td[13]/a/@href').extract()[0].replace(u"\u00A0", " ")
                else:
                    session['textbookLink'] = ""

                if len(sessionXML.xpath('td[14]/a')) == 1:
                    session['courseWebsite'] = sessionXML.xpath('td[14]/a/@href').extract()[0].replace(u"\u00A0", " ")
                else:
                    session['courseWebsite'] = ""

                if len(sessionXML.xpath('td[15]/b/font')) == 1:
                    session['status'] = sessionXML.xpath('td[15]/b/font/text()').extract()[0].replace(u"\u00A0", " ") 
                elif len(sessionXML.xpath('td[15]/font')):
                    session['status'] = sessionXML.xpath('td[15]/font/text()').extract()[0].replace(u"\u00A0", " ")
                else:
                    session['status'] = sessionXML.xpath('td[15]/text()').extract()[0].replace(u"\u00A0", " ")
